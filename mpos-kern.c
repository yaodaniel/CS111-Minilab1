...
//static pid_t do_newThread(process_t  *current, int startFunction);
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		if(current->waiting != NULL) {
		   current->waiting->p_state = P_RUNNABLE;
		   current->waiting->p_registers.reg_eax = current->p_exit_status;
		}
	...
	case INT_SYS_WAIT: {
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
			...
			current->p_state = P_BLOCKED; //Set the current process to be on hold
			proc_array[p].waiting = current; //Tells process with pid p that current is waiting for it to finish
		}
		schedule();
	}
...
copy_stack(process_t *dest, process_t *src)
{
	...
	// Set the values of what we will need
